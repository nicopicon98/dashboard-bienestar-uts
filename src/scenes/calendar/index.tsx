...
const validationSchema = yup.object().shape({
  franjas: yup.array().min(1, "Porfavor, asegurate de ingresar al menos una franja")
})
...
const Calendar = () => {
  ...
  const handleEventClick = (selected: EventClickArg) => {
    if(selected.event.extendedProps.taken){
      alert("Lo sentimos, esa franja ya fue tomada. Si desea cancelar la cita con el estudiante, puede hacerlo en la seccion de citas")
      return
    }
    if (
      window.confirm(
  ...
  useEffect(() => {
    ...
    setSchedule(resp.map(e => {
      return { ...e, id: e.id, title: franjasToTitle(e.franjaId), date: e.date, franjaId: e.franjaId }
    }))
  }, [])
  ...
  return (
    <Box m="20px">
      <Header title="Calendario Citas" subtitle="Maneja el horario de tus citas" />
      {
        loader
          ? <Box display="flex" justifyContent="center" alignItems="center" height="75vh" flexDirection="column">
